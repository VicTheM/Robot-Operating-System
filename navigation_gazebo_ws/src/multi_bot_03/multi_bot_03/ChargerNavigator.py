...
class ChargerNavigator(Node):
    def __init__(self, strRobotName = ""):
        if(strRobotName != ""):
            super().__init__('ChargerNavigator_' + strRobotName)
        ...
        self.robot_base_frame = 'base_link' + self.strRobotNameSlash
        self.camera_frame = 'camera_link' + self.strRobotNameSlash
        ...
        self.publisherCmdVel = self.create_publisher(
            Twist, #TwistStamped, 
            #'/diff_cont/cmd_vel_unstamped' + self.strRobotNameSlash,  # For ROS2 diff. drive controller
            "/cmd_vel" + self.strRobotNameSlash,                       # For Gazebo diff. drive controller
            10)

        self.goal_poses = []
        self.goal_charger = None

        if(self.strRobotNameSlash == ""):
            goal_poses = []
            ...
            self.setChargerPos(goal_charger)
    ...
    def navigation_callback(self):
        ...
        if(self.nBatteryLevel <= 0):
            ...
            self.localization_pose_sub = self.create_subscription(
                ...
                'amcl_pose' + self.strRobotNameSlash, self.amcl_callback, self.navigator.amcl_pose_qos)
