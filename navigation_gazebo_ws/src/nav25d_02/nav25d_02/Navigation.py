...
class Navigation(Node):
    ...
    def printPoseInfo(self):
        ...
        if delta_ms >= 1000:
            ...
            if(self.strRobotName == "" and self.nPointIdx > 0):
                ...
                #self.arrAccelX.append(sqrt(self.imu_data[0]**2 + self.imu_data[1]**2))
    ...
    def imu_callback(self, msg):
        ...
        try:
            ...
            #self.arrAccelX.append(sqrt(ax**2 + ay**2))
    ...
    def showCharts(self):
        if(self.strRobotName == ""):
            ...
            #ax.plot(self.arrChartTime, self.arrAccelX, label='Acceleration', color='red', linestyle='-')
    ...
    def on_magnetometer_callback(self):
        ...
        self.mag_yaw = self.nav.normalize_angle(-np.arctan2(imuMag[0], imuMag[1]))
        ...
        if(self.nCurrentCommand < len(self.cmds) and self.nCycleCount < nMaxCycles):
            ...
            if("imu_gyro" in self.nav.dictSensors):
                if(self.nav.nCurrentKalmanStep % self.nav.imu_step == 0):
                    ...
                    gyro_yaw = self.nav.normalize_angle(gyro_yaw)
            ...
            # fig, ax = plt.subplots()
            # ax.plot(self.arrAccelX, label='Acceleration', color='red', linestyle='-')
...
def main(args=None):
    ...
    #     cmds.append([0,0])
    # for i in range(51):
    #     cmds.append([i / 50., 0.2 * i / 50.])
    # cmds.extend([cmds[-1]]*550)
    # cmds.append([0,0])

    # cmds = [[v, vv] for v, vv in zip(np.linspace(0.001, 1.0, 50), np.linspace(0.001, 0.02, 50))]
