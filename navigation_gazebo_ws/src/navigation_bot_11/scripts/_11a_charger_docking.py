...
class ChargerNavigator(Node):
    def __init__(self):
        ...
        # Uncomment to use OpenCV angle to aruco marker functionality
        ...
        # For reference: here is the data we use:
        ...
        # For reference: this can also be used.
        # self.focal_length = None
        # self.optical_center = None

        # This are parameters extracted from camera.xacro or from single call to 
        ...
        goal_pose.pose.orientation.w = -0.74
        ...
        goal_pose.pose.position.y = 0.36
        goal_pose.pose.position.z = 0.0
        goal_pose.pose.orientation.x = 0.0
        goal_pose.pose.orientation.y = 0.0
        ...
        goal_pose.pose.position.y = 1.31
        goal_pose.pose.position.z = 0.0
        goal_pose.pose.orientation.x = 0.0
        goal_pose.pose.orientation.y = 0.0
        ...
        self.goal_charger.pose.position.x = 6.5
        self.goal_charger.pose.position.y = 5.5
        self.goal_charger.pose.position.z = 0.0
        self.goal_charger.pose.orientation.x = 0.0
        self.goal_charger.pose.orientation.y = 0.0
        self.goal_charger.pose.orientation.z = 0.9
        self.goal_charger.pose.orientation.w = 0.4
    ...
    def publishBatteryStatus(self):
        self.nBatteryLevelCalculated -= 0.05
        msg = BatteryState()
    ...
    def getDockingAction(self):
        ...
        if(self.aruco_distance == None):
            return nLinearSpeed, nAngularSpeed
        ...
        if ids is not None:
            ...
            if(nDistanceToArucoNorm < -0.1):
                print("nDistanceToArucoNorm < 0.1")
                xMid = (topRight[0] - topLeft[0]) / 2
                if(self.lastImage.shape[1] / 2 < xMid):
                    return 0.1, 0.1
                else:
                    return 0.1, -0.1
                
            # Robot is at the right of marker
            elif((topLeft[0] - bottomLeft[0])**2 + (topLeft[1] - bottomLeft[1])**2 < 
                (topRight[0] - bottomRight[0])**2 + (topRight[1] - bottomRight[1])**2):
                print("Robot is at the right of marker")

                if(self.lastImage.shape[1] - topRight[0] > 10):
                    return 0.1, 0.1
                elif(self.lastImage.shape[1] - topRight[0] < 70):
                    return 0.1, -0.1
                else:
                    return 0.1, 0.1

            # Robot is at the left of marker
            else:
                print("Robot is at the left of marker")
                if(topLeft[0] > 10):
                    return 0.1, -0.1
                elif(topLeft[0] < 70):
                    return 0.1, 0.1
    ...
    def navigation_callback(self):
        ...
        if(self.nBatteryLevel <= 0):
            ...
            #     'amcl_pose', self.amcl_callback, self.navigator.amcl_pose_qos)
            ...
            self.printOnce("Going to charger waypoint")
            if(self.navigator.isNavComplete()):
        ...
        # Here we turn around until aruco marker is detected
            ...
            self.printOnce("Looking for charger")
        
            if(self.aruco_end_move_time is None):                       # Start rotating
                print("Looking for charger: rotating 360 degrees")

                # 420 instead of 360 to compensate for slippage.
                angle = 420 #360
                self.arucoRotateByTime(angle, self.angular_speed)
            elif(self.aruco_end_move_time < self.get_clock().now()):     # done rotating 360 degrees, found nothing
                self.arucoRotateByTime(0, 0) # Stop rotating
                print('Unable to find charger')
                self.timer.cancel()
                return False
            else:
                self.aruco_distance = self.getDistanceToAruco(self.lastImage)
                if(self.aruco_distance != None):
                    self.arucoRotateByTime(0, 0) # Stop rotating
                    print("Found marker. Distance: {:.2f}".format(self.aruco_distance))
                    self.navStatus = "going_to_charger"

        elif(self.navStatus == "going_to_charger"):
            nLinearSpeed, nAngularSpeed = self.getDockingAction()
            if(nLinearSpeed == None):
                print("Something wrong: Lost charger")
                return False
            elif(self.aruco_distance < 1):
                self.arucoRotateByTime(0, 0) # Stop
                print("Arrived to charger")
                self.navStatus = "connecting_to_charger"
            else:
                self.printOnce("Going to charger. Distance: {:.2f}, nLinearSpeed: {:.2f}, nAngularSpeed: {:.2f}".
                    format(self.aruco_distance, nLinearSpeed, nAngularSpeed))
