...
class ChargerNavigator(Node):
    def __init__(self):
        ...
        self.goal_charger.pose.orientation.z = 0.9174487146650577
        self.goal_charger.pose.orientation.w = 0.39785406364574616
    ...
    def arucoRotate(self, angleDegrees, angular_speed):
    ...
    def navigation_callback(self):
        ...
        # Here we turn around until aruco marker is detected
            ...
            if(self.aruco_end_move_time is not None 
                ...
                print("Looking for charger: found marker. Distance: {:.2f} meters; angle: {:.0f} degrees".
                    format(self.aruco_distance, self.aruco_angle))
                self.arucoRotate(0, 0) # Stop rotating
            ...
            # TBD: due to slippage, this can be imprecise. Replace with getting position from sensors.
                ...
                angle = 390#360
                self.arucoRotate(angle, self.angular_speed)
            ...
            if(self.aruco_end_move_time is None): 
                ...
                print("Turning to charger line. Turning {:.0f} degrees".format(angle))

                self.arucoRotate(angle, self.angular_speed)
            elif(self.aruco_end_move_time < self.get_clock().now()):
                print("Done turning to charger line")
                self.arucoRotate(0, 0) # Stop rotating
                self.navStatus = "go_to_charger_line"

        elif(self.navStatus == "go_to_charger_line"):
            if(self.aruco_end_move_time is None): 
                print("Going towards charger line, {:.2f} meters".format(self.aruco_distance))
                self.arucoDrive(self.aruco_distance, 1)
            elif(self.aruco_end_move_time < self.get_clock().now()):
                print("Done going towards charger line")
                self.arucoDrive(0, 0)
                self.navStatus = "turning_towards_charger"

        elif(self.navStatus == "turning_towards_charger"):
            if(self.aruco_end_move_time is None): 
                print("Turning towards charger")
                if(self.turnDirection > 0):
                    self.arucoRotate(-90, self.angular_speed)
                else:
                    self.arucoRotate(90, self.angular_speed)
                
            elif(self.aruco_end_move_time < self.get_clock().now()):
                print("Done turning towards charger")
                self.arucoRotate(0, 0) # Stop rotating
                self.navStatus = "going_to_charger"
                
        elif(self.navStatus == "going_to_charger"):
            if(self.aruco_end_move_time is None): 
                print("Going towards charger, {:.2f} meters".format(self.aruco_distance))
                self.aruco_distance = self.getDistanceToAruco(self.lastImage)
                self.arucoDrive(self.aruco_distance - 1, 1)
            elif(self.aruco_end_move_time < self.get_clock().now()):
                print("Done going towards charger line")
                self.arucoDrive(0, 0)
