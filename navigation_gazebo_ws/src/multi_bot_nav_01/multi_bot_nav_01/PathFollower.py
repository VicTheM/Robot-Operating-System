...
GT:             %7.2f  %7.2f  %7.2f  %7.0f\n===\n\n" %  
    ...
    def navigation_callback(self):
        ...
        if(self.navStatus == "stop"):
            ...
            if(self.bPathSet == False):
                ...
                #marker.lifetime = Duration(seconds=10)
                self.marker_pub.publish(marker)
                # marker.action = Marker.DELETE
                # marker_pub.publish(marker)                

                # You can re-publish the entire set with DELETE to clear them all.

                # # Option 2: delete all markers
                # for i in range(10):
                # marker = Marker()
                # marker.id = i
                # marker.action = Marker.DELETE
    ...
    # def publish_path_markers(path_points):
    ...
    #     marker.pose.position.z = point[2] if len(point) >= 3 else 0
