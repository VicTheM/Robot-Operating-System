...
if os.path.exists(strFilePath):
    bUseKalman = True
    from nav25d_04.kalman import KalmanNav
    print(">>>> Use Kalman", flush=True)
else:
    print(">>>> Simulate Kalman", flush=True)
    
from nav25d_04.kalman_utils import euler_from_quaternion
from nav25d_04.kalman_utils import euler_to_quaternion
from nav25d_04.kalman_utils import normalize_angle
from nav25d_04.kalman_utils import DynamicObject
            ...
            if(bUseKalman):
                ...
                if("imu_accel" in self.nav.dictSensors):
                    ...
                    if(self.nav.nCurrentKalmanStep % self.nav.imu_step == 0):
                        ...
                        imuVel = self.ukf.x[3:5]

                    z.extend(imuVel)

                if("imu_gyro" in self.nav.dictSensors):
                    if(self.nav.nCurrentKalmanStep % self.nav.imu_step == 0):
                        #gyro_yaw = normalize_angle(kalman_prev[2] + self.imu_data[2] * self.nav.dt)
                        #gyro_yaw = normalize_angle(self.nav.ukf.x[2] + self.imu_data[2] * self.nav.dt)
                        gyro_yaw = normalize_angle(self.imu_data[2])

                        bNeedUpdate = True
                    else:
                        gyro_yaw = self.nav.ukf.x[5]
...
def main(args=None):
    ...
    # Commented out as wouldn't work if nMaxCycles > 1
    for i in range(51):
        cmds.append([i / 25., 0.])
